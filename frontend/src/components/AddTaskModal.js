...
const AddTaskModal = ({ isAddTaskModalOpen, setAddTaskModal, projectId = null, taskId = null, edit = false, refreshData }) => {
    ...
    useEffect(() => {
        if (edit && isAddTaskModalOpen) {
            axios.get(`https://leon-prod-ui.vercel.app/project/${projectId}/task/${taskId}`)
                .then((res) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
        if (!edit) {
            axios.post(`https://leon-prod-ui.vercel.app/project/${projectId}/task`, { requestor, title, description: desc })
                .then((res) => {
            ...
            axios.put(`https://leon-prod-ui.vercel.app/project/${projectId}/task/${taskId}`, { requestor,  title, description: desc })
                .then((res) => {
                    setAddTaskModal(false)
                    ...
                    <div className="fixed inset-0 flex items-center justify-center p-4 w-screen h-screen">
                        ...
                        >
                            <Dialog.Panel className="rounded-md bg-white w-6/12">
                                ...
                                <div className='mb-3'>
                                        <label htmlFor="requestor" className='block text-gray-600'>Requestor</label>
                                        <input value={requestor} onChange={(e) => setRequestor(e.target.value)} type="text" className='border border-gray-300 rounded-md w-full text-sm py-2 px-2.5 focus:border-indigo-500 focus:outline-offset-1 focus:outline-indigo-400' placeholder='Task title' />
                                    </div>
                                    <div className='mb-3'>
                                    ...
                                    <div className='mb-2'>
                                        ...
                                        <textarea value={desc} onChange={(e) => setDesc(e.target.value)} className='border border-gray-300 rounded-md w-full text-sm py-2 px-2.5 focus:border-indigo-500 focus:outline-offset-1 focus:outline-indigo-400' rows="10" placeholder='Task description'></textarea>
                                    </div>
